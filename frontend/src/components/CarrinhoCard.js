...
export default function CarrinhoCard({produto, ...props}){
    ...
    */
    ...
    return(
        <div className="mt-5 group relative flex items-center rounded-lg p-4 text-sm/6 bg-white shadow-md">
            <div className="flex h-12 w-12 flex-none items-center justify-center rounded-lg">
              ...
              <a href="detalhes_prod.html" className="absolute w-12 h-12"></a>
            </div>
            <div className="flex-auto border-x-4 border-dark px-3 mx-3">
              <a href="./detalhes_prod.html" className="block text-xl font-bold text-primaryBlue">
                {produto.prodName}
              </a>
              <p className="mt-0.5 text-lg font-semibold text-secondaryBlue">Unid. R$ {produto.prodPrice.toFixed(2)}</p>
            ...
            <div>
                <button type="button" onClick={() => handleQtdClick(-1)}
                    className="size-8 inline-flex justify-center items-center gap-x-2 text-md  rounded-md border-1 border-accentBlue bg-primaryBlue text-white shadow-sm hover:bg-accenteBlue focus:outline-none focus:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none" tabIndex="-1" aria-label="Decrease">
                ...
                <input className="p-0 w-8 bg-transparent border-0 text-gray-800 text-center focus:ring-0 [&::-webkit-inner-spin-button]:appearance-none [&::-webkit-outer-spin-button]:appearance-none dark:text-white" type="number" aria-roledescription="Number field" 
                    value={qtd} onChange={(e)=>handleQtdChange(e.target.value)} data-hs-input-number-input=""/>
                
                <button type="button" onClick={() => handleQtdClick(1)}
                    className="size-8 inline-flex justify-center items-center gap-x-2 text-md  rounded-md border-1 border-accentBlue bg-primaryBlue text-white shadow-sm hover:bg-accenteBlue focus:outline-none focus:bg-gray-50 disabled:opacity-50 disabled:pointer-events-none" tabIndex="-1" aria-label="Increase">
                    <svg className="shrink-0 size-3.5" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round">
                        <path d="M5 12h14"></path>
