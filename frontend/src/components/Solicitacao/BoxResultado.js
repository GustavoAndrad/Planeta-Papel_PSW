...
export default function BoxResultado({status,setStatus}){
    ...
    return(<>
        <div class={`${status===null? "w-2/3 mx-auto min-w-72 py-5 flex flex-col items-center bg-white rounded-3xl border-2 border-accentBlue":"invisible h-0"}  `}>
        ...
        <div class={`${status===true? "w-2/3 mx-auto min-w-72 py-5 flex flex-col items-center bg-white rounded-3xl border-2 border-accentBlue":"invisible h-0"}  `}>
            <h1 class="text-xl font-semibold text-center">Informar data limite</h1>
            <input type="date" id="data-coleta" class="mt-3 overflow-hidden whitespace-nowrap text-ellipsis block p-2.5 w-30 z-20 text-sm text-gray-900 bg-gray-50 rounded-lg border-s-gray-50 border-s-2 border border-gray-300 focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-s-gray-700  dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:border-blue-500" required />
            <a class="mt-3 text-sm font-semibold text-center">O horário padrão é SEMPRE de<br/>10:00 às 18:00</a>
            <button type="submit" class="mt-3 min-w-[200px] hover:bg-secondaryBlue w-3/4 bg-primaryBlue text-white bg-primary-600 hover:bg-primary-700 focus:ring-4 focus:outline-none focus:ring-primary-300 font-medium rounded-full text-2xl px-5 py-2.5 text-center dark:bg-primary-600 dark:hover:bg-primary-700 dark:focus:ring-primary-800">
                Confirmar</button>
        </div>
